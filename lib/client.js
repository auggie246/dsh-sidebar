      ...
      function BottomPanel(props) {
        ...
        return h('section', {
          ...
          h('div', { className: 'rsb-tabstrip' },
            ...
            tabs.tabs.map((t) => {
              // Chip text comes from the tab's type entry (ticket #7); a
              // type without a strip — the URL tab — keeps its URL as both
              // title and label.
              const def = TAB_TYPES[t.type]
              const chip = def && def.strip ? def.strip(t) : { title: t.url, label: t.url }
              return h('div', {
                key: t.id,
                ...
                h('span', { className: 'rsb-tab-label' }, chip.label),
                h('button', {

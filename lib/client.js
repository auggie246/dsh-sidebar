/*
 * dsh-git-sidebar — browser half (composition client plugin).
 *
  ...
  factory: (require) => {
    ...
    function apply(ctx) {
      ...
      ctx.effect(() => {
        ...
        el.id = 'dsh-git-sidebar-styles'
        el.textContent = CSS
        document.head.appendChild(el)
        return () => { el.remove() }
      }, `${PACKAGE}: stylesheet`)

      ctx.slots.inject('details', () => ctx.slots.register({ name: 'details' }, (props) => h(SidebarPanel, props)))
      ctx.slots.inject('shell.overlay', () => ctx.slots.register({ name: 'shell.overlay', id: 'rside-rail', label: 'Workspace sidebar' }, () => h(Rail)))
    }

...
// Issue #27: one file's change against HEAD. `diff` is empty when git reports
// no change, which is an untracked file's case: the client shows the Text
// Preview rather than an empty frame.
export const DiffResultCodec = codec(`${PACKAGE}/DiffResult`, (value) => {
  const v = record(value, 'diff result')
  onlyKeys(v, ['diff'], 'diff result')
  return { diff: string(v.diff, 'gitDiff.diff') }
})

// Issue #21: one entry of the Explorer Card's directory listing. kind is
// the coarse type the tree needs to draw a caret and decide what a click
// does: dir expands, file previews, link defers to its target.
...
export const DirListResultCodec = codec(`${PACKAGE}/DirListResult`, (value) => {
...
export class GitSidebarGateway extends TypertRemoteService {
  ...
  gitDiff(cwd, path) { return this.controller.gitDiff(cwd, path) }
  listDir(cwd, path) { return this.controller.listDir(cwd, path) }

...
export function apply(ctx) {
  ...
  const controller = {
    ...
    async readFile(cwdArg, pathArg) {
      ...
      const realRes = await ctx.shell.run(ctx.shell.resolve({ command: 'realpath -- ' + shq(abs), timeoutMs: 15000 }))
      ...
      const sizeRes = await ctx.shell.run(ctx.shell.resolve({ command: 'wc -c ' + shq(target), timeoutMs: 15000 }))
      ...
      const catRes = await ctx.shell.run(ctx.shell.resolve({ command: 'cat -- ' + shq(target), timeoutMs: 15000 }))

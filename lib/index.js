...
export function apply(ctx) {
  ...
  const controller = {
    ...
    // `-printf '%y\t%f\n'` yields the type char and name per entry in one
    // command. The line format cannot carry names containing tabs or
    // newlines; such lines are skipped fail-closed rather than guessed.
    async listDir(cwdArg, pathArg) {
      ...
      const entries = []
      for (const line of text(findRes.stdout).split('\n')) {

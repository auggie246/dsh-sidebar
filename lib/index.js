...
export function apply(ctx) {
  ...
  const controller = {
    ...
    async readFile(cwdArg, pathArg) {
      ...
      const sizeRes = await ctx.shell.run(ctx.shell.resolve({ command: 'wc -c ' + shq(abs), timeoutMs: 15000 }))
      ...
      if (Number.isFinite(size) && size > READ_FILE_LIMIT) throw failure('file is larger than the 2 MB preview limit')
      const catRes = await ctx.shell.run(ctx.shell.resolve({ command: 'cat -- ' + shq(abs), timeoutMs: 15000 }))
      if (catRes.exitCode !== 0) throw failure(text(catRes.stderr).trim() || 'cannot read file')
      return { content: text(catRes.stdout) }
    },

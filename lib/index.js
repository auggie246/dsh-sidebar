...
// Explorer (issue #21): one listDir reply carries at most 1000 entries; the
// true count rides along in total and the card renders a "+N more" row.
...
export function apply(ctx) {
  ...
  const controller = {
    ...
    // diff. The path gets readFile's exact confinement walk — plain-string
    // based (no node:path) so the dynamic twin mirrors it literally, and
    // re-checked on the realpath so a symlink inside the Working Repository
    // pointing outside it cannot escape. `--no-color` pins the format whatever
    // the user's color.ui says, so the client parser never sees an escape
    // sequence. An unborn HEAD has nothing to compare against, and git says so
    // on stderr; that is an empty diff, not a failure, because the client
    // shows the Text Preview for a file with no change. The reply is capped
    // like readFile's, so one enormous change cannot flood the channel.
    async gitDiff(cwdArg, pathArg) {
      const rootRaw = cwdOf(cwdArg)
      const root = rootRaw.length > 1 ? rootRaw.replace(/\/+$/, '') : '/'
      const path = String(pathArg == null ? '' : pathArg).trim()
      if (!path) throw failure('no path given')
      ...
      const real = text(realRes.stdout).trim()
      if (root !== '/' && real !== root && real.indexOf(root + '/') !== 0) throw failure('path escapes the working repository')
      const r = await git(cwdOf(cwdArg), 'diff HEAD --no-color -- ' + shq(real), { timeoutMs: 30000 })
      if (r.code !== 0) {
        ...
        throw failure(r.err || 'git diff failed')
      }
      if (r.out.length > READ_FILE_LIMIT) throw failure('diff is larger than the 2 MB preview limit')
      return { diff: r.out }
    },

    // Explorer (issue #21): list ONE directory level of the workspace the
    // card is rooted at. The listing path gets readFile's exact confinement
    // walk — plain-string based (no node:path) so the dynamic twin mirrors
    // it literally, re-checked on the realpath so a symlinked directory
    // pointing outside the root cannot escape. The listing itself is one
    // GNU find: the shell service spawns commands directly (no pipes), and
    // `-printf '%y%f\0'` yields one NUL-terminated record per entry — type
    // char then name — in one command. NUL because a POSIX file name can
    // contain a tab or a newline but never a NUL byte, so "nothing hidden"
    // holds for hostile names too. Unparsable records are skipped rather
    // than guessed. Entries are sorted (folders first, then case-
    // insensitive names) and capped afterwards, so the kept slice is the
    // sorted-first 1000, not find's arbitrary readdir order.
    async listDir(cwdArg, pathArg) {
      const rootRaw = cwdOf(cwdArg)
      const root = rootRaw.length > 1 ? rootRaw.replace(/\/+$/, '') : '/'
      const path = String(pathArg == null ? '' : pathArg).trim()
      if (path.indexOf('\0') !== -1) throw failure('path contains a NUL byte')
      ...
      const realRes = await ctx.shell.run(ctx.shell.resolve({ command: 'realpath -- ' + shq(abs), timeoutMs: 15000, sandboxPolicy: repoPolicy(root) }))
      if (realRes.exitCode !== 0) throw failure(text(realRes.stderr).trim() || 'cannot list directory')
      const real = text(realRes.stdout).trim()
      if (root !== '/' && real !== root && real.indexOf(root + '/') !== 0) throw failure('path escapes the working repository')
      const findRes = await ctx.shell.run(ctx.shell.resolve({
        command: 'find ' + shq(real) + ' -maxdepth 1 -mindepth 1 -printf ' + shq('%y%f\\0'),
        timeoutMs: 15000,
        sandboxPolicy: repoPolicy(root),
      }))
      if (findRes.exitCode !== 0) throw failure(text(findRes.stderr).trim() || 'cannot list directory')
      const entries = []
      for (const record of text(findRes.stdout).split('\0')) {
      ...
      return { entries: entries.slice(0, LIST_DIR_LIMIT), hasMore: total > LIST_DIR_LIMIT, total }
    },

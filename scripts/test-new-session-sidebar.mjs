...
hookState.clear()
tree = renderFunction(overlay, newSessionProps)
const openSidebarToggles = findClass(tree, 'rsb-header-toggles-blank')
assert.ok(openSidebarToggles.props.className.split(' ').includes('rsb-header-toggles-blank-open'), 'blank-session toggles must track the open Sidebar')
assert.match(stylesheet, /\.rsb-header-toggles-blank-open \{[^}]*right: calc\(var\(--rsb-panel-w\) \+ 28px\)/, 'open blank-session toggles must sit beside the Sidebar')
